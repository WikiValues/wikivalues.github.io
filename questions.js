questions = [{
		"question": "Начинать новый википроект лучше всего с создания большого количества правил или расписывания политики поведения на вики.",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": -5,
			"scty": 0,
       "dipl": 0
		}
	},
	{
		"question": "Те, кто больше трудятся, кто вносят больше вклада в развитие вики, должны получать больше возможностей (в т. ч., например, особые привилегии).",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Чем больше прав и возможностей у каждого участника, тем общество справедливей и честней.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Любая власть должна быть оспоримой.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Любая власть должна бать заменяемой: никакие бюрократы, администраторы и модераторы не должны править на википроекте вечно.",
		"effect": {
			"govt": 5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "На википроектах должны открыто проводиться голосования; каждый обычный участник должен иметь право голоса и право свободно участвовать в таком голосовании.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "На википроектах должны открыто проводиться обсуждения и коллективно решаться вопросы с обычными участниками.",
		"effect": {
			"govt": 10,
			"cmnt": 5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Чем больше участников будет опрошено, тем больше шансов на достижение наилучшего решения.",
		"effect": {
			"govt": 5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Администрация может иметь в голосованиях преимущества по сравнению с обычными участниками (например, приравнивая один свой голос к пяти голосам обычных участников).",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Важно, чтобы администрация придерживалось мнения большинства, даже если оно спорное/сомнительное/неверное.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Никто, кроме меня, не может адекватно представлять свою точку зрения. Моё слово — закон.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Я поддерживаю иерархическое устройство вики, как естественное и неизбежное. Отсутствие процесса формирования элит (высшего класса) — признак неразвитости или застойности общества.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Превосходство одних участников и неполноценность других являются врожденной и объективной реальностью.",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Народ (в виде обычных участников) в целом не может управлять целой вики. Эту функцию должны брать на себя определённые выделяющиеся участники или элита общества.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Оторванность от народа неизбежно возникает в иерархических и не-демократических обществах; поэтому, чтобы решить эту проблему, каждому администратору надо быть таким же открытым перед обычными участниками, как и перед другими администраторами/модераторами.",
		"effect": {
			"govt": 5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Такие статусы, как «бюрократ», «администратор», «модератор» не делают участника выше и главнее остальных. Это лишь инструменты, дающие доступ к определённым функциям интерфейса вики, или же возможность выдавать/снимать с участников те или иные статусы (в случае с бюрократом).",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Слежка со стороны администрации необходима в современном вики-обществе.",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Администрация не должна вмешиваться в личную социально-общественную жизнь участников на вики, а также в их конфликты, споры.",
		"effect": {
			"govt": 5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Администрация, по сравнению с обычными участниками, может иметь исключения касательно правил вики (т.е. те правила, которые обязаны соблюдать все обычные участники, администрация имеет право не соблюдать в определенных ситуациях).",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Большей свободы для участников можно достичь сокращением некоторых функций администрации и/или сделав правила менее жёсткими и строгими.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 5,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Чем сильнее и строже руководство на вики, тем эффективней, лучше и быстрее будет проводиться работа.",
		"effect": {
			"govt": -5,
			"cmnt": 5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Самоуправление — хорошая форма организации работы для всех участников википроекта, в отличии от непосредственного и принудительного контроля за участниками со стороны вышестоящих органов власти.",
		"effect": {
			"govt": 10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Большинство участников принимает неверные решения в отношении тех или иных вопросов, поэтому необходимо направить их в определенную сторону, задать вектор движения, чтобы они следовали определенной идеологии, принципам, правилам (даже используя силу, принуждение).",
		"effect": {
			"govt": -10,
			"cmnt": 5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Независимо от идеологических взглядов, необходимо быть на стороне администрации своей вики.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Администрация имеет право проводить репрессии и блокировать нежелательных участников, которые высказывают противоположную точку зрения или критикуют действия самой администрации.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": -10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Цензура, устанавливаемая администрацией, необходима для обеспечения внутренней и внешней безопасности википроекта, его стабильности, а также для ограничения распространения нежелательных сведений.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Нет ничего плохого в формировании культа личности определённого администратора.",
		"effect": {
			"govt": -10,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Важно ставить общественные интересы над личными.",
		"effect": {
			"govt": 0,
			"cmnt": 10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Нельзя пренебрегать желаниями отдельно взятых участников, даже если их предложения никак не помогают развитию общества.",
		"effect": {
			"govt": 0,
			"cmnt": -5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Все участники абсолютно разные, так что нельзя рассматривать их как единую массу, как единое сообщество.",
		"effect": {
			"govt": 0,
			"cmnt": -5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Развитие общества вполне возможно без существования единых для всех целей.",
		"effect": {
			"govt": 0,
			"cmnt": -10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Спонтанные действия участников более эффективны, чем тщательно спланированные.",
		"effect": {
			"govt": 0,
			"cmnt": -10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Каждый участник должен иметь право на свободные решения от вмешательств администрации и поступать так, как считает нужным он сам.",
		"effect": {
			"govt": 5,
			"cmnt": -10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Важно, чтобы у участников википроекта были общие интересы.",
		"effect": {
			"govt": 0,
			"cmnt": 5,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Работая вместе, участники могут добиться большего, чем если бы работали поодиночке.",
		"effect": {
			"govt": 0,
			"cmnt": 10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Собирая участников в единое сообщество, гораздо больше шанс сплотить всех, cдружиться, найти общие интересы, чем если бы все занимались работой поодиночке и независимо друг от друга.",
		"effect": {
			"govt": 0,
			"cmnt": 10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "В первую очередь надо уделять внимание целому сообществу, а то только каким-то отдельно взятым личностям.",
		"effect": {
			"govt": 0,
			"cmnt": 10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Работая вместе с остальными, точно так же, как и все, участники рискуют потерять свою индивидуальность в толпе.",
		"effect": {
			"govt": 0,
			"cmnt": -10,
			"jstc": 0,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Не стоит чрезмерно завышать сроки банов участников (например, выдавать баны сроком на несколько месяцев, полгода или больше).",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Не стоит упускать из виду даже самые мелкие нарушения правил участниками (например, одно матерное слово в большом комментарии). Даже такие нарушения стоят серьёзного рассмотрения, ровно как и более серьезные нарушения.",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": -5,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Не лишним станет создание на вики списка нарушителей, в который будут заноситься любые участники, когда-либо нарушавшие правила на википроекте, включая даже самые незначительные нарушения.",
		"effect": {
			"govt": -5,
			"cmnt": 0,
			"jstc": -10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "С теми, кто нарушал правила неоднократно, бесполезно пытаться находить общий язык.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": -10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Чтобы избежать рецидивизма, мы должны банить нарушителей как можно раньше.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": -5,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Важно делать нарушителей своеобразными «примерами», которым не стоит следовать.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": -5,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Участнику, по поведению которого видно, что он потенциально опасен и может натворить много вреда для общества, лучше превентивно выдать блокировку, чем допускать риск совершения им преступлений.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": -10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "К нарушителю-рецедивисту вполне оправдано негативное отношение со стороны остальных участников, и неприязнь в обществе.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": -10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Насколько бы серьёзное нарушение не совершил участник в первый раз, даже в таком случае, недопустимо ему сразу выдавать перманентный бан.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 5,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Правосудие должно всегда учитывать контекст и прошлое нарушителей, и соответствующим образом адаптировать свои наказания.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 10,
			"scty": 0,
      "dipl": 0
		}
	},
  {
		"question": "Участников, отбывших свой срок блокировки, следует реинтегрировать в общество.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "В случае конфликта двух администраторов, важно будет попытаться помирить их, оставить в обществе обоих, ведь в противном случае, снятие всех флагов с одного из администраторов и его уход с википроекта, может серьёзно повредить состоянию всего википроекта.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 10,
			"scty": 0,
      "dipl": 0
		}
	},
	{
		"question": "Лучшая жизнь общества придёт путём прогресса, развития, изменений в википроекте.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 10,
      "dipl": 0
		}
	},
	{
		"question": "Важно сохранять традиции, принципы прошлого.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -10,
      "dipl": 0
		}
	},
	{
		"question": "Мы ни при каких обстоятельствах не должны менять что-либо, что доказало свою пользу и работоспособность многими годами ранее.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -10,
      "dipl": 0
		}
	},
	{
		"question": "Преследовать прогресс зачастую бывает нерационально и слишком затратно.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -10,
      "dipl": 0
		}
	},
	{
		"question": "Подключение к работе википроекта новых функций — это в любом случае положительный процесс, который означает развитие, изучение и познание нового.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 5,
      "dipl": 0
		}
	},
	{
		"question": "Важно постоянно следовать в ногу со временем, и в случае чего, незамедлительно делать обновления тех или иных элементов википроекта.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 10,
      "dipl": 0
		}
	},
	{
		"question": "Необходимо вернуть возможность включения достижений на википроекте администраторами через Wiki Features, без нужды запрашивать подключение данного расширения у сотрудников ФЭНДОМА.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -5,
      "dipl": 0
		}
	},
	{
		"question": "Удаление текущих расширений, на которых построены стены обсуждений, комментарии и профили участников на ФЭНДОМЕ, и замена их новыми расширениями — необходимая и неотъемлемая часть масштабного обновления ФЭНДОМА.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 10,
      "dipl": 0
		}
	},
	{
		"question": "Я поддерживаю обновление скинов ФЭНДОМА (например, замену Monaco на Oasis), как прогрессивный и революционный шаг в оформлении.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 10,
      "dipl": 0
		}
	},
	{
		"question": "Обсуждения — если не лучше форумов, то по крайне мере это неплохая альтернатива им.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 5,
      "dipl": 0
		}
	},
	{
		"question": "Планируемое удаление форумов и полная замена их на обсуждения — крайне предательский и подлый ход со стороны ФЭНДОМА.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -10,
      "dipl": 0
		}
	},
	{
		"question": "Планируемое объединение ФЭНДОМА и Gamepedia является совершенно лишним и ненужным действием.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": -10,
      "dipl": 0
		}
	},
	{
		"question": "Классический вики-чат должен быть упразднён, а ему на замену должна прийти интеграция ФЭНДОМА с Дискордом.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 5,
      "dipl": 0
		}
	},
	{
		"question": "Участники должны иметь право регистрироваться и находиться впоследствии на ФЭНДОМЕ, даже если им меньше 13 лет.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 5,
      "dipl": 0
		}
	},
  {
		"question": "Я активно занимаюсь правками на многих википроектах, не только на каком-то одном.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 10
		}
	},
	{
		"question": "Дипломатические отношения с другими вики выгодны и полезны.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 5
		}
	},
	{
		"question": "Желательно поддерживать дружественные отношения между википроектами-единомышленниками, либо википроектами на схожую тематику.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 10
		}
	},
	{
		"question": "Википроекты схожих тематик должны объединяться в викисоюзы или альянсы.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 10
		}
	},
  {
		"question": "Я не откажусь в случае викидружбы от взаимной рекламы моего википроекта и чужого.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 5
		}
	},
  {
		"question": "В случае объединения википроектов в викисоюз или альянс, я не буду против подчинения их единым правилам, организации совместной работы и прочего.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 10
		}
	},
  {
		"question": "Важно ставить интересы целых групп вики выше интересов отдельных википроектов. Цели какого-либо одного википроекта не должны ставиться выше всех остальных.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 10
		}
	},
  {
		"question": "Администраторам википроектов не будет лишним принимать участие в различных обсуждениях на центральной Вики Сообщества по тем или иным вопросам.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 5
		}
	},
  {
		"question": "Администраторы разных вики должны сотрудничать всякий раз, когда это выгодно им обоим.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 5
		}
	},
  {
		"question": "Я готов добровольно, в случае чего, помочь даже совершенно незнакомому мне википроекту.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": 5
		}
	},
  {
		"question": "Викисоюзы и альянсы могут представлять угрозу суверенитету отдельных и независимых википроектов.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": -10
		}
	},
	{
		"question": "Викисоюзы, как правило, не несут в себе никакой реальной пользы и существуют только на словах. Они не приводят ни к чему хорошему, и часто совершенно не нужны википроектам.",
		"effect": {
			"govt": 0,
			"cmnt": 0,
			"jstc": 0,
			"scty": 0,
      "dipl": -10
		}
	}
];
